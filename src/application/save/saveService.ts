...
export class SaveService {
  ...
  public async save(profileId: ProfileId, next: SaveEnvelopeV1): Promise<void> {
    if (!validateSaveEnvelope(next, profileId).valid)
      throw new Error('Cannot save an invalid envelope.');
    ...
    if (!verified || !validateSaveEnvelope(verified, profileId).valid)
      throw new Error('Save verification failed after write.');
  ...
  public async load(
    ...
    for (const slot of ['current', 'previous', 'lastKnownGood'] as const) {
      ...
      if (candidate && validateSaveEnvelope(candidate, profileId).valid)
        return { save: candidate, recoveredFrom: slot };
    }
    return { save: null, recoveredFrom: null };
  }

  public deleteProfile(profileId: ProfileId): Promise<void> {
