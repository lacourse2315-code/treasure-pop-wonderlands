...
export class IndexedDbSaveStore implements SaveStorePort {
  ...
  public async write(profileId: ProfileId, slot: SaveSlot, save: SaveEnvelopeV1): Promise<void> {
    if (!validateSaveEnvelope(save, profileId).valid)
      throw new Error('Refusing to persist an invalid save.');
    const db = await openDatabase();
    const transaction = db.transaction(STORE, 'readwrite');
    transaction
      .objectStore(STORE)
      .put({ key: key(profileId, slot), profileId, slot, save } satisfies StoredSave);
  ...
  public async deleteProfile(profileId: ProfileId): Promise<void> {
    ...
    for (const slot of ['current', 'previous', 'lastKnownGood'] as const)
      transaction.objectStore(STORE).delete(key(profileId, slot));
    await transactionDone(transaction);
    db.close();
  }
}

function key(profileId: ProfileId, slot: SaveSlot): string {
